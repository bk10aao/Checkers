...
namespace Application {
	...
	public class ControllerV12 : MonoBehaviour
	{
		...
		private bool canMoveDownAndRight (int x, int y) {
			PlayerPiece piece = gameBoard.returnPlayerPiece (x, y);
			if ((piece != null) && (piece.playerNo == 1 || piece.isKing == true) && ((y + 1 < 8) && (x + 1 < 8)) 
			   && gameBoard.returnPlayerPiece (x + 1, y + 1) == null) {
				return true;
			}
			return false;
		}
		...
		private bool canMoveDownAndLeft (int x, int y) {
			PlayerPiece piece = gameBoard.returnPlayerPiece (x, y);
			if ((piece != null) && (piece.playerNo == 1 || piece.isKing == true) && ((y - 1 > -1) && (x + 1 < 8)) 
			   && gameBoard.returnPlayerPiece (x + 1, y - 1) == null) {
				return true;
			}
			return false;	
		...
		private bool canMoveUpAndRight (int x, int y) {
			PlayerPiece piece = gameBoard.returnPlayerPiece (x, y);
			if ((piece != null) && (piece.playerNo == 2 || piece.isKing == true) && ((y + 1 < 8) 
			   && (x - 1 > -1)) && gameBoard.returnPlayerPiece (x - 1, y + 1) == null) {
				return true;
			}
			return false;
		}
		...
		private bool canMoveUpAndLeft (int x, int y) {
			PlayerPiece piece = gameBoard.returnPlayerPiece (x, y);
			if ((piece != null) && (piece.playerNo == 2 || piece.isKing == true) && ((y - 1 > -1) 
			   && (x - 1 > -1)) && gameBoard.returnPlayerPiece (x - 1, y - 1) == null) {
				return true;
			}
			return false;
		...
		private bool canTakeDownAndRight (int x, int y) {
			...
			if (currentPiece != null && (currentPiece.playerNo == 1 || currentPiece.isKing == true) && ((y + 2 < 8) && (x + 2 < 8))) {
				...
				if (enemy != null && enemy.GetPlayerNumber () == getOpponent (currentPiece.GetPlayerNumber ())) {
		...
		private Boolean playerHasTakeableMoves (int playerNumber) {
			int i = 0, j = 0;
			while (j < 8) {
				while (i < 8) {
					if ((canTakeDownAndRight (i, j) || canTakeDownAndLeft (i, j) 
					   || canTakeUpAndRight (i, j) || canTakeUpAndLeft (i, j)) 
					   && gameBoard.returnPlayerPiece (i, j).playerNo == playerNumber) {
						return true;
					}
					i++;
				}
