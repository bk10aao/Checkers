	...
	public class ControllerV3 : MonoBehaviour
	{
		...
		private void takeDown(int x, int y, int temp1, int temp2, GameObject playerPiece) {
			if (canTakeDownAndRight (x, y) || canTakeDownAndLeft(x,y)) {
				Debug.Log ("Player can take down");
				PlayerPiece piece = gameBoard.returnPlayerPiece (x, y);
				gameBoard.removePiece (x, y);
				gameBoard.removePiece (x + 1, temp1);
				gameBoard.AddPlayerPiece (piece, x + 2, temp2);
		...
		/**private void takePiece(int x, int y, int enemyPosX, int enemyPosZ, int moveToSpaceX, int moveToSpaceY, GameObject playerPiece) {
			PlayerPiece piece = gameBoard.returnPlayerPiece (x, y);
			gameBoard.removePiece (x, y);
			gameBoard.removePiece ((x + enemyPosX), (y + enemyPosZ));
			gameBoard.AddPlayerPiece (piece, x + moveToSpaceX, y + moveToSpaceY);
			if (piece.playerNo == 2 && piece.isKing == false && x - 2 == 0) {
				transformPieceToKing (piece, playerPiece);
			} else if (piece.playerNo == 1 && piece.isKing == false && x + 2 == 7)
				transformPieceToKing (piece, playerPiece);
			else {
				//do nothing
			}
